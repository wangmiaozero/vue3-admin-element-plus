    ...
    service.interceptors.response.use(
        response => {
            ...
            if (custom_options.code_message_show && response.data && response.data.code != 1) {
                ...
                return Promise.reject(response.data); // code不等于1, 页面具体逻辑就不执行了
            }
...
function httpErrorStatusHandle(error) {
    ...
    if (error && error.response) {
        switch (error.response.status) {
            case 302: message = '接口重定向了！';removeAccessToken(); break;
            case 400: message = '参数不正确！'; break;
            case 401: message = '您未登录，或者登录已经超时，请先登录！'; removeAccessToken(); break;
